...
/**
 * Earliest Date {@link ConflictResolutionFunction}: Returns the earliest date.
 * 
 * @author Robert Meusel (robert@dwslab.de)
 *
 */
public class EarliestDate<RecordType extends Matchable & Fusable>
	...
	public FusedValue<DateTime, RecordType> resolveConflict(Collection<FusableValue<DateTime, RecordType>> values) {
		...
		if (values.size() == 0) {
			return new FusedValue<>((DateTime) null);
		} else {
			...
			for (FusableValue<DateTime, RecordType> value : values) {
				if (value != null) {
					if (dateTime == null) {
						...
						if (dateTime.isAfter(value.getValue())) {
							dateTime = value.getValue();
						}
					}
				}
