...
public abstract class MatchableFactory<RecordType extends Matchable> {
	...
	/**
	 * returns the child node with the given node name from the given node.
	 * 
	 * @param node
	 *            the node containing the data
	 * @param childName
	 ...
	 * @return
	 */
	protected <ItemType extends Matchable> List<ItemType> getObjectListFromChildElement(Node node, String childName,
			String objectNodeName, MatchableFactory<ItemType> factory, String provenanceInfo) {
		...
		for (int j = 0; j < children.getLength(); j++) {
			...
			if (child.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE && child.getNodeName().equals(childName)) {
				...
				// prepare a list to hold all values
				List<ItemType> values = new ArrayList<>(child.getChildNodes().getLength());
