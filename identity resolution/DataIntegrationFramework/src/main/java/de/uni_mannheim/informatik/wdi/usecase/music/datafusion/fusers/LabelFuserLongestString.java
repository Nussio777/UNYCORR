...
/**
 * {@link AttributeValueFuser} for the labels of {@link Release}s.
 * 
 * @author Robert Meusel (robert@dwslab.de)
 * 
 */
public class LabelFuserLongestString extends AttributeValueFuser<String, Release> {

	public LabelFuserLongestString() {
		super(new LongestString<Release>());
	}
	...
	public void fuse(RecordGroup<Release> group, Release fusedRecord) {
		...
		fusedRecord.setLabel(fused.getValue());

		// add provenance info
		fusedRecord.setAttributeProvenance(Release.LABEL, fused.getOriginalIds());
	}

	@Override
	public boolean hasValue(Release record) {
		return record.hasValue(Release.LABEL);
	}
